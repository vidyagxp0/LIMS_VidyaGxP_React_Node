...
function StandardProtocol() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://lims-api.mydemosoftware.com/get-all-lims/sMStandardProtocol`
      );
  ...
  const handleEditSave = async (updatedData) => {
    try {
      ...
      const response = await axios.put(
        `https://lims-api.mydemosoftware.com/manage-lims/update/sMStandardProtocol/${updatedData.uniqueId}`,
        dataToSend
      );
      if (response.status === 200) {
        setData((prevData) =>
  ...
  const handleAdd = async (newStandardProtocol) => {
    try {
      const response = await axios.post(
        `https://lims-api.mydemosoftware.com/manage-lims/add/sMStandardProtocol`,
        {
          ...newStandardProtocol,
