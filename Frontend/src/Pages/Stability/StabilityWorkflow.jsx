...
const StabilityWorkFlow = () => {
  ...
  const generatePDF = async (sampleId) => {
    ...
    try {
      const response = await fetch(
        `https://lims-api.mydemosoftware.com/stability-generate-report/${sampleId}`
      );
  ...
  const handleDelete = (item) => {
    ...
    axios
      .delete(
        `https://lims-api.mydemosoftware.com/delete-lims/sLSamplePA/${item.uniqueId}`
      )
      .then((response) => {
  ...
  return (
    <div>
      ...
      <div className="relative top-28 overflow-auto">
        <table className="min-w-full bg-white border border-gray-200 shadow-lg">
          <thead>
            ...
            <tr className="bg-slate-600 text-white">
              ...
              <td className="border px-4 py-2">
                Photostability Testing results
              </td>
